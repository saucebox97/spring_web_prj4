...
import static com.project.web_prj.util.LoginUtils.LOGIN_COOKIE;
import static com.project.web_prj.util.LoginUtils.getAutoLoginCookie;
...
public class MemberService {
        ...
        if (foundMember != null) {
            if (encoder.matches(inputData.getPassword(), foundMember.getPassword())) {
                ...
                if (inputData.isAutoLogin()) {
                    ...
                    keepLogin(foundMember.getAccount(), session, response); // foundMember.getAccount()로 계정명받아옴
                }
    ...
    private void keepLogin(String account, HttpSession session, HttpServletResponse response) {
        ...
        // 2. 쿠키 설정 (수명, 사용 경로)
        int limitTime = 60 * 60 * 24 * 90; // 90일짜리
        c.setMaxAge(limitTime);
        c.setPath("/"); // 전체경로 / 어디서든 들어올수있게
        // 3. 로컬에 쿠키 전송
